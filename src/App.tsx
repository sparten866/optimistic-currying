...
// ── Helper: compute paid/remaining from payments array ──
const getPaid = (e) =>
  (e.payments || []).reduce((s, p) => s + (parseFloat(p.amount) || 0), 0);
const getRemaining = (e) => Math.max(0, e.totalAmount - getPaid(e));
const getStatus = (e) => {
...
function StatusBadge({ entry }) {
  ...
  if (st === "Paid")
    return <span style={badge("#15803d", "#dcfce7")}>✓ Paid</span>;
  if (st === "Partial")
    return <span style={badge("#7e22ce", "#f3e8ff")}>◑ Partial</span>;
  const d = daysDiff(entry.dueDate);
  if (d < 0)
    return (
      <span style={badge("#b91c1c", "#fee2e2")}>Overdue {Math.abs(d)}d</span>
    );
  if (d === 0)
    return <span style={badge("#c2410c", "#ffedd5")}>Due Today</span>;
  if (d <= 3)
    return <span style={badge("#b45309", "#fef3c7")}>Due in {d}d</span>;
...
function TypeBadge({ type }) {
  ...
  return (
    <span style={{ ...badge(s.color, s.bg), fontSize: 10 }}>{s.label}</span>
  );
}
...
function DashCard({ icon, label, val, sub, bg }) {
  return (
    <div
      style={{
        background: bg,
...
// ── Payment Modal ───────────────────────────────────────
function PaymentModal({ entry, onClose, onAdd }) {
  ...
  const submit = () => {
    ...
    if (a > remaining + 0.01) {
      alert("Amount exceeds remaining balance of " + PKR(remaining));
      return;
    }
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            style={{
              background: remaining > 0 ? "#fff7ed" : "#f0fdf4",
              borderRadius: 10,
              padding: "12px 14px",
            }}
          >
            ...
            >
              <div>
                ...
                <input
                  ...
                  placeholder={PKR(remaining).replace("PKR\u00a0", "")}
                  style={inp}
                />
              </div>
              <div>
...
function CashFlowTab({ entries }) {
  ...
  const days = Array.from({ length: 28 }, (_, i) => {
    ...
    const due = entries.filter(
      (e) => e.dueDate === dateStr && getStatus(e) !== "Paid"
    );
    return {
      d,
      dateStr,
      due,
      total: due.reduce((s, e) => s + getRemaining(e), 0),
    };
  });
  ...
  const weekLabels = weeks.map((w, i) => {
    const start = w[0].d.toLocaleDateString("en-GB", {
      day: "numeric",
      month: "short",
    });
    const end = w[6].d.toLocaleDateString("en-GB", {
      day: "numeric",
      month: "short",
    });
  ...
  const getDayBg = (day, i) => {
    ...
    if (day.total > 0 && i > 0) {
      ...
      const bgs = ["#eff6ff", "#f0fdf4", "#fdf4ff", "#fefce8"];
      return { bg: bgs[wk], border: "1px solid #e2e8f0", label: null };
    }
    return { bg: "#fafafa", border: "1px solid #f1f5f9", label: null };
  };
  ...
  return (
    <div>
      ...
      >
        ...
        >
          ...
          <div style={{ color: "#64748b", fontSize: 11, marginTop: 4 }}>
            {days.filter((d) => d.total > 0).length} payment days
          </div>
        </div>
        {weeks.map((w, i) => {
          const wkTotal = w.reduce((s, day) => s + day.total, 0);
          const colors = ["#1e40af", "#15803d", "#7e22ce", "#b45309"];
          const bgs = [
      ...
      {weeks.map((week, wi) => {
        ...
        const wkColors = ["#1e40af", "#15803d", "#7e22ce", "#b45309"];
        return (
          <div key={wi} style={{ marginBottom: 20 }}>
            ...
            >
              ...
              <div style={{ color: "#34d399", fontWeight: 800, fontSize: 14 }}>
                {wkTotal > 0 ? (
                  ...
                  <span style={{ color: "#475569" }}>No payments</span>
                )}
              </div>
            </div>
            {/* Days */}
            ...
            >
              {week.map((day, di) => {
                ...
                const { bg, border, label, labelColor } = getDayBg(
                  day,
                  globalIdx
                );
                const dayName = day.d.toLocaleDateString("en-GB", {
                  weekday: "long",
                });
                const dateFormatted = day.d.toLocaleDateString("en-GB", {
                  day: "numeric",
                  month: "long",
                  year: "numeric",
                });
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      padding: "12px 18px",
                      background: bg,
                      borderBottom: di < 6 ? "1px solid #e2e8f0" : "none",
                      opacity: isWeekend && day.total === 0 ? 0.5 : 1,
                    }}
                  >
                    {/* Date column */}
                    <div
      ...
      {grandTotal === 0 && (
        ...
        >
          ...
          <div style={{ fontSize: 16, fontWeight: 600 }}>
            No payments due in the next 4 weeks
          </div>
        </div>
      )}
    </div>
...
export default function App() {
  ...
  const handleSubmit = () => {
    ...
    if (form.entryType === "purchase" && (!form.weight || !form.rate)) {
      toast$("Fill Weight & Rate", "error");
      return;
    }
    if (form.entryType !== "purchase" && !form.amount) {
      toast$("Enter invoice amount", "error");
      return;
    }
    const entry = {
      ...
      item:
        form.entryType === "purchase" ? form.item : TYPES[form.entryType].label,
  ...
  const addPayment = (entryId, amount, date, note) => {
    const payment = {
      id: Date.now().toString(),
    ...
    const updated = entries.map((e) => {
      ...
      const newStatus =
        paidAmt >= e.totalAmount ? "Paid" : paidAmt > 0 ? "Partial" : "Pending";
      ...
      entries.map((e) =>
        e.id === id
          ? {
              ...
              payments: e.status === "Paid" ? [] : e.payments || [],
            }
          : e
      )
    );
  ...
  const exportCSV = () => {
    ...
    a.href = "data:text/csv;charset=utf-8," + encodeURIComponent(csv);
    a.download =
      "SupplierPayments_" + new Date().toISOString().slice(0, 10) + ".csv";
    a.click();
    toast$("CSV exported!");
  };
  ...
  const soon = pending.filter((e) => {
    const d = daysDiff(e.dueDate);
    return d >= 0 && d <= 7;
  });
  ...
  const bySupplier = pending.reduce((a, e) => {
  ...
  const filtered = entries
    .filter((e) => {
      const tm =
        filterType === "All"
          ? true
          : (e.entryType || "purchase") === filterType;
      const status = getStatus(e);
      const sm =
        filterStatus === "All"
          ? true
          : filterStatus === "Overdue"
          ? status !== "Paid" && daysDiff(e.dueDate) < 0
          : filterStatus === "Due Soon"
          ? status !== "Paid" &&
            daysDiff(e.dueDate) >= 0 &&
            daysDiff(e.dueDate) <= 7
      ...
      >
        ...
        >
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
            <div style={{ textAlign: "right" }}>
              ...
              <div
                style={{
                  color: overdue.length > 0 ? "#f87171" : "#34d399",
                  fontSize: 18,
                  fontWeight: 800,
                }}
              >
        ...
        <div style={{ display: "flex", gap: 4 }}>
          {[
            ...
            >
              {label}
              {id === "master" && entries.length > 0 && (
                <span
                  style={{
                    background: tab === "master" ? "#1e40af" : "#334155",
                    color: "#fff",
                    borderRadius: 10,
                    padding: "1px 7px",
                    fontSize: 10,
          ...
          >
            <div>
                ...
                >
                  {Object.entries(TYPES).map(([key, info]) => (
                    <button
                      ...
                      style={{
                        background:
                          form.entryType === key ? info.bg : "#f8fafc",
                        border:
                          "2px solid " +
                          (form.entryType === key ? info.color : "#e2e8f0"),
                        borderRadius: 12,
                        padding: "12px 10px",
                        cursor: "pointer",
                        textAlign: "left",
                    ...
                    >
                      <div
                        style={{
                          ...
                          fontSize: 13,
                          color:
                            form.entryType === key ? info.color : "#475569",
                          marginBottom: 3,
                        }}
                      >
              ...
              >
                ...
                >
                  {cfg.emoji} {cfg.label} — Entry Details
                </h2>
                <div
                  style={{
                    display: "grid",
                ...
                >
                  <Field label="Supplier / Vendor Name *" span={2}>
                    <input
                      ...
                      readOnly={!!TYPES[form.entryType]?.supplierFixed}
                      placeholder="e.g. Malik Livestock"
                      style={{
                        ...inp,
                        background: TYPES[form.entryType]?.supplierFixed
                          ? "#f8fafc"
                          : "#fff",
                      }}
                    />
                  ...
                  {form.entryType !== "purchase" ? (
                    <>
                      <Field label="Shipment Reference">
                        <input
                          ...
                          onChange={(e) => f("shipmentRef", e.target.value)}
                          placeholder="e.g. SHP-2025-041"
                          style={inp}
                        />
                      </Field>
                      <Field label="Invoice / Certificate No.">
                        <input
                          ...
                          onChange={(e) => f("invoiceRef", e.target.value)}
                      ...
                      <Field
                        label={
                          "Amount (PKR) *" +
                          (form.entryType === "ihc" ? " — default 79,870" : "")
                        }
                      >
                        <input
                          type="number"
                    ...
                    <>
                      <Field label="Item / Description">
                        <input
                          value={form.item}
                          onChange={(e) => f("item", e.target.value)}
            ...
            <div>
              ...
              >
                ...
                {form.entryType === "purchase" ? (
                  <>
                    <PRow
                      ...
                      val={
                        form.weight
                          ? Number(form.weight).toLocaleString() + " kg"
                          : "—"
                      }
                    />
                    <PRow
                      ...
                      val={
                        form.rate
                          ? "PKR " + Number(form.rate).toLocaleString() + "/kg"
                          : "—"
                      }
                    />
                  </>
                ...
                <div
                  style={{ margin: "16px 0", borderTop: "1px solid #1e3a5f" }}
                />
                <div
                  style={{
                    display: "flex",
        ...
        {!loading && tab === "master" && (
          <div>
            ...
            >
              ...
              {["All", "purchase", "ihc", "aviation"].map((t) => (
                <button
                  ...
                  style={{
                    ...
                    border:
                      "1px solid " + (filterType === t ? "#0f172a" : "#e2e8f0"),
                    borderRadius: 20,
                    padding: "5px 14px",
                    fontSize: 12,
                    fontWeight: 600,
              ...
              {["All", "Pending", "Partial", "Paid", "Overdue", "Due Soon"].map(
                (s) => (
                  <button
                    key={s}
            ...
            {filtered.length === 0 ? (
              ...
              <div style={{ overflowX: "auto" }}>
                ...
                >
                  ...
                  <tbody>
                    {filtered.map((e, i) => {
                      const isBuy = (e.entryType || "purchase") === "purchase";
                      ...
                      return (
                        ...
                        >
                          <td style={{ padding: "11px 14px" }}>
                            <TypeBadge type={e.entryType || "purchase"} />
                          </td>
                          <td
                            style={{
                              padding: "11px 14px",
                          ...
                          <td
                            style={{ padding: "11px 14px", color: "#475569" }}
                          >
                            {isBuy ? (
                              e.item
                            ) : (
                          ...
                          <td style={{ padding: "11px 14px" }}>
                            <div style={{ display: "flex", gap: 6 }}>
                              ...
                              <button
                                ...
                                title={
                                  eSt === "Paid" ? "Mark Pending" : "Mark Paid"
                                }
                                style={{
                                  background:
                                    eSt === "Paid" ? "#fef3c7" : "#dcfce7",
                                  border: "none",
                                  borderRadius: 6,
                                  padding: "4px 8px",
                                  cursor: "pointer",
                  ...
                  <tfoot>
                    <tr style={{ background: "#0f172a", color: "#fff" }}>
                      ...
                      >
                        {PKR(filtered.reduce((s, e) => s + e.totalAmount, 0))}
                      </td>
                      <td
                        style={{
                          padding: "12px 14px",
                      ...
                      >
                        {PKR(filtered.reduce((s, e) => s + getPaid(e), 0))}
                      </td>
                      <td
                        style={{
                          padding: "12px 14px",
                      ...
                      >
                        {PKR(filtered.reduce((s, e) => s + getRemaining(e), 0))}
                      </td>
                      <td colSpan={4}></td>
                    </tr>
                  </tfoot>
          ...
          <div>
            ...
            >
              ...
              <DashCard
                icon="🚨"
                label="Overdue"
                val={PKR(overdue.reduce((s, e) => s + getRemaining(e), 0))}
              ...
              <DashCard
                ...
                val={PKR(partial.reduce((s, e) => s + getRemaining(e), 0))}
                sub={partial.length + " partial"}
